...
const nameValidationHandler = (event) => {
  ...
  for (i of content.split(" ")) {
    ...
    message[0]
      ? allMessage.push(`name: ${i} <br> ${message.join("<br>")}`)
      : "";
  }
...
const lastValidationHandler = (event) => {
  ...
  for (i of content.split(" ")) {
    ...
    message[0]
      ? allMessage.push(`surname: ${i} <br> ${message.join("<br>")}`)
      : "";
  }
...
const addressValidationHandler = (event) => {
  ...
  if (
    ...
    message.push(
      `supports spanish and portugues leters, numbers and (-.,()'"°/) simbols`
    );
  }
